...
8. `CasesView` 의 학대유형·위험도·키워드 필터는 `03_UI_UX.md` S02 명세에 없는 항목입니다(1-3). **유지하실 계획이면 Contract 에 반영**하고, 아니면 명세 범위로 줄이는 편이 낫습니다. 어느 쪽인지 알려주시면 그에 맞추겠습니다.
9. `03_UI_UX.md` §5 가 **"위험 확정" · "AI 판정"** 같은 표현을 금지합니다. `detected: true` 는 모델 내부 값이므로 화면에는 **"관련 신호"** · **"추가 확인 필요"** 로 표기해 주세요. 색상만으로 위험도를 표현하지 않는 규칙도 같은 절에 있습니다.
