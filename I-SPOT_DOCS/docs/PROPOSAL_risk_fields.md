...
> 실제 판정 초안은 §7-4 에 있습니다. 유형별 임계값이 다르고 모델 버전마다 또 달라서
...
- **임계값을 Contract 에 상수로 박으면 안 됩니다.** 모델이 바뀌면 같이 바뀝니다.
...
단일 확신도 기준(`>= 0.7`)은 **쓸 수 없습니다.** 위에서 본 대로 유형별 기준선이 다르고, 모델 버전이 바뀌면 값 자체가 뒤집히기 때문입니다(§7-2). 모델이 계산한 `detected` 를 기준으로 삼습니다.
